...
size_t unlz2k(std::ifstream &src, std::ofstream &dest, size_t srcSize,
  ...
  size_t read = src.tellg() - start;
  if (read != srcSize) {
    std::cerr << std::format("Expected to read 0x{:X} bytes, but read 0x{:X}",
                             srcSize, static_cast<size_t>(src.tellg()));
    throw 3;
  }
  if (bytesWritten != destSize) {
    std::cerr << std::format("Expected to write 0x{:X} bytes, but wrote 0x{:X}",
